...
<template>
  ...
  <div class="flex align-items-stretch flex-wrap">
    <info
      class="flex align-items-center justify-content-center font-bold border-round m-1"
      style="height: 2.5em; width: 55%; background-color: blue; color: white"
      msg="Comprobante de la entrega"
    />
  </div>
  <section
    class="block bg-primary font-bold text-center p-4 border-round"
    style="height: 4em"
    v-if="showLoad"
  >
    {{ load }}
  ...
  <div class="container">
    <div class="contain">
      <h1>Datos cliente</h1>
      <div class="customer">
        <h4 style="color: blue">Cliente:</h4>
        {{ response.dict.cliente }}
        <h4 style="color: blue">DNI:</h4>
        {{ response.dict.dni }}
        <h4 style="color: blue">Direccion:</h4>
        {{ response.dict.address }}
        <h4 style="color: blue">Telefono:</h4>
        {{ response.dict.phone }}
        <h4 style="color: blue">Estado pedido:</h4>
    ...
    <div class="contain">
      <h1>Datos pedido</h1>
      <div class="customer">
        <h4 style="color: blue">Fecha de entrega:</h4>
        {{ response.order_data.delivery_date }}
        <h4 style="color: blue">Numero de pedido:</h4>
        {{ response.order_data.order_number }}
        <h4 style="color: blue">Hora limite:</h4>
        {{ response.order_data.delivery_time }}
        <h4 style="color: blue">Franja Horaria:</h4>
        {{ response.order_data.delivery_time_interval }}
      </div>
    </div>
    <div class="contain">
      <h1>Bultos pedido</h1>
      <div class="order_drawers">
        <section class="block_section">
          ...
          <div class="block_div flex flex-row flex-wrap justify-content-center align-items-center">
            <div class="block">
              <h4 class="flex flex-row flex-wrap justify-content-center align-items-center">
                Frio
              </h4>
              <span class="span_block">{{ response.drawers.cold }}</span>
            </div>
            <div class="block">
              <h4 class="flex flex-row flex-wrap justify-content-center align-items-center">
                Congelado
              </h4>
              <span class="span_block">{{ response.drawers.frozen }}</span>
            </div>
            <div class="block">
              <h4 class="flex flex-row flex-wrap justify-content-center align-items-center">
                Seco
              </h4>
              <span class="span_block">{{ response.drawers.dry }}</span>
            </div>
            <div class="block">
              <h4 class="flex flex-row flex-wrap justify-content-center align-items-center">
                Fuera de cajon
              </h4>
              <span class="span_block">{{ response.drawers.out_of_drawers }}</span>
            </div>
          </div>
        </section>
        <section class="block_section">
          ...
          <div class="block_div flex flex-row flex-wrap justify-content-center align-items-center">
            <div class="block">
              ...
              <span class="span_block">{{ response.bags.cold }}</span>
            </div>
            <div class="block">
              <h4 class="flex flex-row flex-wrap justify-content-center align-items-center">
                Congelado
              </h4>
              <span class="span_block">{{ response.bags.frozen }}</span>
            </div>
          </div>
        </section>
        <section class="block_section">
          ...
          <div class="block_div flex flex-row flex-wrap justify-content-center align-items-center">
            <div class="block">
              <h4 class="flex flex-row flex-wrap justify-content-center align-items-center">
                {{ response.orders_packages.substitutions }}
              </h4>
            </div>
          </div>
        </section>
    ...
    <div class="contain">
      <div class="textarea_1">
        <span class="p-float-label">
          <Textarea
            id="myTextarea"
            v-model="response.delivery_note.note"
            rows="3"
    ...
    <div class="contain">
      <div class="return">
        <span class="p-float-label">
          <Textarea
            id="textarea"
            v-model="response.returned_product.return_reason"
            rows="2"
            style="margin-top: 5px; width: 100%"
          />
          <label>Devoluciones y motivo de devoluciones:</label>
          <small id="textarea-help"
            >apunta aqui los productos devueltos y el o los motivo(s) de la devolucion</small
          >
        </span>
        <br />
        ...
        <div class="flex">
          <InputNumber
            v-model="response.returned_product.unity"
            inputId="integeronly"
            style="width: 96%; height: 4em"
          />
          <Button
            icon="pi pi-plus"
            aria-label="Submit"
            @click="response.returned_product.unity++"
  ...
  <br />
  <Message severity="error" :sticky="sticky" :life="6000" v-if="showErrorMessage">{{
    errorMessage
  }}</Message>
  <Message severity="success" :sticky="sticky" :life="1000" v-if="showSuccess">{{
    successMessage
  }}</Message>
  <div class="contain">
    <div class="receptor">
      ...
      <div class="card flex justify-content-center">
        <div class="flex flex-column gap-2">
          ...
          <InputText
            id="username"
            v-model="response.receptor_data.name"
...
import info from '@/components/infoText.vue'
...
const errorMessage = ref(
  'Es importante comprobar los siguientes datos: si hay devoluciones especificar la cantidad y tanto el Dni y nombre del receptor no pueden estar vacios'
)
...
onMounted(() => {
  ...
  if (piniaStore.customerStatus == 'Entregado') {
    ...
    console.log(ele, el, dn, us, btn)
  }
})
const beforeLeave = onBeforeRouteLeave(() => {
  const answer = window.confirm(
    'Dou you really want to leave, may check if you have some unsaved changes!'
  )
...
=======
const leavePath = onBeforeRouteUpdate(() => {
  piniaStore.getIdentity() && beforeLeave == true
    ? router.push({
        path: '/optionPage',
        name: 'optionPage'
      })
    : piniaStore.router.push({
>>>>>>> d6e4e5350e9d249c70b523fb9231c91aa20c0549
        path: '/loginPage',
        name: 'loginPage'
      })
  console.log('To optionPage or loginPage')
})
...
const loadCustmer = async () => {
  try {
    ...
    response.dict = await piniaStore.data.json()
    let isString =
      Object.prototype.toString.call(response.dict) === '[object String]' ? true : false
    console.log('isString', isString)
    response.order_data = response.dict.order_data
    response.delivery_note = response.dict.delivery_note
    response.orders_packages = response.dict.orders_packages
    ...
    fetching.value = false
    !error.value != null && response.dict != null
...
watch(
  () => [response.receptor_data.DNI, response.receptor_data.name],
  () => {
...
const onClicked = async () => {
  ...
  if (
    (response.returned_product.return_reason != '' && response.returned_product.unity != '') ||
    (response.returned_product.return_reason == '' &&
      response.returned_product.unity == '' &&
      response.receptor_data.name != '' &&
      response.receptor_data.DNI != '')
  ) {
    styleObject.backgroundColor = '#59c88b'
    submit.value = 'Enviado'
    console.log(
