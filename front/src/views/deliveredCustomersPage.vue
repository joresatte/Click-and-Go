...
<template>
  ...
  <section class="customers">
    <section
      ...
      style="background-color: #707070; padding: 1em; border-radius: 0.5em"
    >
      <section class="flex justify-content-start">
        <IconField>
          <InputIcon class="pi pi-search" style="color: blue" />
          <InputText v-model="filteredText" placeholder="Search" style="width: 8em; color: blue" />
        </IconField>
      </section>
    ...
    <div
      class="block bg-primary font-bold text-center p-4 border-round mb-3"
      style="margin-top: 10%"
      v-if="showLoad"
    >
      ...
      <div class="col-12" v-for="(item, index) in filteredData" :key="index">
        <div
          ...
          v-if="
            ...
            item.status.toLowerCase().includes(filteredText.toLowerCase())
              ? true
              : false
          "
        >
          ...
          >
            <div class="flex flex-column align-items-center sm:align-items-start gap-3">
              <img
                class="w-4 sm:w-5rem xl:w-6rem shadow-2 block xl:block mx-auto border-round"
                :src="item.picture"
                alt="image client"
              />
            ...
            <div class="flex sm:flex-column align-items-center sm:align-items-end gap-3 sm:gap-2">
              ...
              <Button
                ...
                :loading="loading"
                class="btn-class-btn"
                @click="onclicked(item._id, item.status)"
              />
            </div>
          </div>
        </div>
...
const loadData = async () => {
  try {
    ...
    let isString =
      Object.prototype.toString.call(response.value) === '[object String]' ? true : false
    console.log('isString', isString)
  } catch (errors) {
    error.value = errors
  } finally {
    fetching.value = false
    !error.value != null && response.value != null
...
function resetTimer() {
  ...
  /* Set a new interval */
  timer.value = setInterval(startIdleTimer, 3600000)
}
